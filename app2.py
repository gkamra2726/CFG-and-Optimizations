...
#  GCC SEMANTIC CHECK 
def gcc_semantic_check(code):
    """Run gcc -Wall -Wextra -fsyntax-only on original code (with #includes) and parse output."""
    import subprocess, tempfile, shutil
    results = []

    if not shutil.which("gcc"):
        return [{"line": 0, "col": 0, "level": "note",
                 "message": "gcc not found on this system — install gcc to enable compiler checks"}]
    ...
    try:
        proc = subprocess.run(
            ["gcc",
             ...
             "-Wuninitialized", # use before init
             ...
             "-fsyntax-only",   # don't compile, just check
             tmp_path],
        ...
        for line in output.splitlines():
            # gcc format:  filename:line:col: level: message
...
with col2:
    if st.button("🚀 Run Analysis"):
        try:
            ...
            dead = analyze(ast)
            gcc_issues = gcc_semantic_check(code)  # pass original code so gcc sees #includes
            ...
            st.write(dead if dead else "None")
            ...
            st.subheader("Semantic Analysis (GCC)")
            if not gcc_issues:
                st.success("GCC: No warnings or errors found.")
            else:
                errors   = [i for i in gcc_issues if i["level"] == "error"]
                warnings = [i for i in gcc_issues if i["level"] == "warning"]
                notes    = [i for i in gcc_issues if i["level"] == "note"]
                for i in errors:
                    st.error(f"Line {i['line']}:{i['col']}  ❌  {i['message']}")
                for i in warnings:
                    st.warning(f"Line {i['line']}:{i['col']}  ⚠️  {i['message']}")
                for i in notes:
                    st.info(f"Line {i['line']}:{i['col']}  ℹ️  {i['message']}")
