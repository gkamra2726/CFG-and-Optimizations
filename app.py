...
# ---------------- GRAPH COLOR ----------------
def write_colored_dot(cfg, filename, highlight_nodes=None):
...
def get_target_label(selected_line):
    line = selected_line.split("//")[0].strip().rstrip(";")
...
def get_highlight_nodes(cfg, target_label):
    ...
    for node in cfg.nodes:
        ...
        if base.startswith(target_label):
            nodes.append(node)
    return nodes
...
with col2:
    if st.button("🚀 Run Analysis"):
        try:
            ...
            if target_label:
                highlight_nodes_after = get_highlight_nodes(cfg_after, target_label)
            else:
